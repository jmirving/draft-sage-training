...
### Default dataset scope (all operations)
- Default input scope for training and dataset-driven generators is cleaned
  `2025 + Jan 2026` at `.tmp/prodata-2025-plus-2026-01-clean` unless explicitly overridden.
- This default applies to training runs and artifact generation scripts that consume
  processed pro data.
...
- Generation behavior: role priors are now built with causal patch weighting by default
  (target patch uses only that patch and earlier patches), with a strong same-major
  weight boost and mild within-major recency weighting.
- Path: `--role-priors-dir` with files named like `15.01.json` (generated by `scripts/generate_role_priors.py`).
- Requires: processed player rows + champion mapping; validates role ordering.
...
```
python scripts/generate_champion_eligibility.py \
  --champion-mapping-path /path/to/champion-mapping.json \
  --output-path data/eligibility/champion-eligibility.json
```
`--input-dir` defaults to `.tmp/prodata-2025-plus-2026-01-clean`; override only when intentionally changing dataset scope.
